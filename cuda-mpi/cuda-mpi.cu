...
extern matElement* runCuda(int mpi_rank, int rowSizePerWorkstation, float *h_a, float *h_b)
{   
    int num_gpus;
    cudaGetDeviceCount(&num_gpus);
    int rowSizePerGpu = ((rowSizePerWorkstation - 1) / num_gpus) + 1;
    ...
    dim3 blockPerGrid(gridSize_x , gridSize_y);
    ...
    for (int j = 0; j < num_gpus; j++)
    {
        for (int i = 0; i < (gridSize_x) * (gridSize_y); i++)
        {
            ...
            {   
                ...
                minElement[j * num_gpus].row = h_minValueFromEachBlock[j][i].row + ((mpi_rank == 0 ? j : (j + 2)) * rowSizePerGpu);         //each gpu uses it's own row, col addressing from 0. but gpu 1 maybe hadling rows from 2048 which it considers as rows from 0 and finds its min value index so we scale it for each gpu here
    ...
    {   
        cudaSetDevice(j);
        find2Min<<<blockPerGrid, threadsPerBlock, 0, streams[j]>>>((minElement[j * num_gpus].row - ((mpi_rank == 0 ? j : (j + 2)) * rowSizePerGpu)), minElement[j * num_gpus].col, d_c[j], d_minValueFromEachBlock[j]);

        CHECK(cudaMemcpyAsync(h_minValueFromEachBlock[j], d_minValueFromEachBlock[j], (gridSize_x) * (gridSize_y) * sizeof(matElement), cudaMemcpyDeviceToHost, streams[j]));
    }
    ...
    {    
        for (int i = 0; i < (gridSize_x) * (gridSize_y); i++)
        {
            if(h_minValueFromEachBlock[j][i].value < minElement[j * num_gpus + 1].value)
            {
                ...
                minElement[j * num_gpus + 1].row = h_minValueFromEachBlock[j][i].row + ((mpi_rank == 0 ? j : (j + 2)) * rowSizePerGpu);
                minElement[j * num_gpus + 1].col = h_minValueFromEachBlock[j][i].col;
            }
        }
    }
    
    static matElement finalMinElements[2];
